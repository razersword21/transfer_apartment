...
daily_routine_prompt = """根據"人物記憶"中的重要事情和"今日時間"，生成你今天的行程，符合以下"安排行程規則"
<安排行程規則>
...
design_action_prompt = """根據提供"人物記憶"、"人物行程"、"觀察事項"、"當前時間"和"當前地點"決定你接下來要做的動作內容
其中要包含動作的地點和互動物件名稱，遵循以下"動作決定規則"
<動作決定規則>
- 動作描述不要包含人物想法、後續行程內容和原因
...
- "execution time"表示互動物件的時間，單位為秒，若"object"為"Nothing"則"execution time"回傳0s
- 使用繁體中文
<動作決定規則>
...
<目前地點>{current_location}</目前地點>
<當前時間>{current_time}</當前時間>"""
create_thought_prompt = """根據提供的"人物記憶"、"觀察事項"、"當前時間"和"目前地點"生成你目前的想法，遵循以下"想法規則"
<想法規則>
- 用一段話表示想法內容
- 不要包含自己名字，可包含其他人名字
- 使用繁體中文
...
create_dialogue = """<談話對象>{interactive_character}</談話對象>
<人物記憶>{memory}</人物記憶>
<觀察事項>{observes}</觀察事項>
<目前地點>{current_location}</目前地點>
<當前時間>{current_time}</當前時間>
